...
class SubscribeHandler:
    ...
    def payment_received(update: Update, context: CallbackContext):
        ...
        subscription_end = datetime.utcnow() + timedelta(days=3 * 30)  # For testing purposes, you can change this to the actual duration of the subscription
        update_user_access(user_id, has_access=True, subscription_end=subscription_end, subscription_type=subscription_type)

        # Add logging
        logging.info(f"User {username} (ID: {user_id}) subscribed")

        # Confirm the payment
        context.bot.send_message(user_id, "Thank you for subscribing! You now have access to all features.")
    ...
    monthly_subscription_handler = CallbackQueryHandler(
        lambda update, context: send_invoice(update, context, SubscribeHandler.subscription_plans[0]),
        pattern="^subscribe_monthly_subscription$"
    )
    three_monthly_subscription_handler = CallbackQueryHandler(
        lambda update, context: send_invoice(update, context, SubscribeHandler.subscription_plans[1]),
        pattern="^subscribe_3_monthly_subscription$"  # Update the pattern string here
    )
    yearly_subscription_handler = CallbackQueryHandler(
        lambda update, context: send_invoice(update, context, SubscribeHandler.subscription_plans[2]),
