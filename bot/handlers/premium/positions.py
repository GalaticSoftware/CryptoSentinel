    ...
    def cache(seconds):
        def decorator_cache(func):
            ...
            def wrapper_cache(*args, **kwargs):
                ...
                if key not in wrapper_cache.cache or time.time() - wrapper_cache.cache[key][1] > seconds:
            ...
            for position in positions:
                ...
                if long:
                    if position_value_usd >= whale_filter_size:
                        long_rows.append([symbol, entry_price, mark_price, pnl, roe, position_value_usd_str, leverage])
                        total_long_above_threshold += position_value_usd
                    else:
                        total_long_below_threshold += position_value_usd
                elif short:
                    if position_value_usd >= whale_filter_size:
                        short_rows.append([symbol, entry_price, mark_price, pnl, roe, position_value_usd_str, leverage])
                ...
                if not is_cached:
                    if long:
                        if position_value_usd >= whale_filter_size:
                            if [symbol, entry_price, mark_price, pnl, roe, position_value_usd_str, leverage] not in long_rows:
                                long_rows.append([symbol, entry_price, mark_price, pnl, roe, position_value_usd_str, leverage])
                                total_long_above_threshold += position_value_usd
                        else:
                            total_long_below_threshold += position_value_usd
                        ...
                        if position_value_usd >= whale_filter_size:
                            if [symbol, entry_price, mark_price, pnl, roe, position_value_usd_str, leverage] not in short_rows:
                                short_rows.append([symbol, entry_price, mark_price, pnl, roe, position_value_usd_str, leverage])
                                total_short_above_threshold += position_value_usd
                        else:
                            total_short_below_threshold += position_value_usd
            ...
            if not is_cached:
                ...
                if long_rows:
                    ...
                    for i, row in enumerate(long_rows):
                        pnl = float(row[3].replace(',', '')) if isinstance(row[3], str) else row[3]
                        amount = float(row[5].replace(',', '')) if isinstance(row[5], str) else row[5]
                ...
                if encrypted_uid in PositionsHandler.position_output_dict:
                    update.message.reply_text(PositionsHandler.position_output_dict[encrypted_uid], parse_mode=ParseMode.HTML)
                else:
                    continue
