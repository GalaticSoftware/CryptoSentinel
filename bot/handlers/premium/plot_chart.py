...
class ChartHandler:
    ...
    async def plot_chart(update: Update, context: CallbackContext):
        ...
        try:
            chart_file = await PlotChart.plot_ohlcv_chart(symbol, time_frame)
        except Exception as e:
            logger.exception("Error while plotting the OHLCV chart")
            await update.message.reply_text("Error while plotting the OHLCV chart. Please try again later.")
            return
        ...
        try:
            ...
            await update.message.reply_text("Error while sending the chart. Please try again later.")
            return
